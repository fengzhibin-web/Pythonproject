...
# 添加更详细的数据验证
def validate_npz_data(file_path):
    with np.load(file_path) as data:
        print("\n详细数据验证:")
        print(f"节点特征统计 - 均值: {np.mean(data['node_feat']):.4f}, 标准差: {np.std(data['node_feat']):.4f}")
        print(f"边数据唯一性 - 唯一边数量: {len(np.unique(data['edges'], axis=0))}")
        print(f"标签分布: {np.bincount(data['labels'])}")
        print(f"节点-图像映射有效性: {len(np.unique(data['node_image_map']))}张图像")